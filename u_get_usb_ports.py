...
def get_usb_ports():
    ...
    if platform.system() == 'Windows':
        ...
        if arch[0] == '32bit':
            backend = usb.backend.libusb1.get_backend(find_library=lambda x: "USB/libusb/x86/libusb-1.0.dll")  # 32-bit DLL, pick right one based on Python installation
        elif arch[0] == '64bit':
            backend = usb.backend.libusb1.get_backend(
                find_library=lambda x: "USB/libusb/x64/libusb-1.0.dll")  # 64-bit DLL
        else:
            raise Exception('Invalid platform. System must be a 32bit or 64bit architecture.')

    # FIND MANAGERS
    found_info = usb.core.show_devices(idVendor=MANAGER_VENDOR_ID, idProduct=MANAGER_PRODUCT_ID)
    ...
    for line in manager_lines:
        ...
        if bus and address:
            unique_name = 'usb_{}_{}_{}_{}'.format(MANAGER_VENDOR_ID, MANAGER_PRODUCT_ID, bus, address)
    ...
    # FIND LASERS
    found_info = usb.core.show_devices(idVendor=LASER_VENDOR_ID, idProduct=LASER_PRODUCT_ID)
    ...
    for line in laser_lines:
        ...
        emulation = False
        if bus and address:
            unique_name = 'usb_{}_{}_{}_{}'.format(LASER_VENDOR_ID, LASER_PRODUCT_ID, bus, address)
            found_vortran_devices[unique_name] = dict()
        ...
        if emulation:
            unique_name = 'usb_{}_{}_{}_{}'.format("EmuVendID", "EmuProdID", "NoBus", "FakeAddy")
            found_vortran_devices[unique_name] = dict()
            found_vortran_devices[unique_name]['vendor_id'] = LASER_VENDOR_ID
            found_vortran_devices[unique_name]['product_id'] = LASER_PRODUCT_ID
            found_vortran_devices[unique_name]['bus'] = bus
    ...
    if tmp_match_address:
        if len(tmp_match_address.groups()) == 1:
            try:
                ...
                print(
                    'Unable to get a bus in utility get_usb_ports')
    return bus, address
